...
// In production, you’d avoid the global variable, such as by passing the variable to functions that need it or by wrapping it in a struct.
var db *sql.DB
...
func main() {
	...
	// Call DB.Ping to confirm that connecting to the database works. At run time, sql.Open might not immediately connect, depending on the driver. 
	...
	// Hard-code ID 4 here to test the query.
	alb, err := albumByID(4)
	if err != nil {
		log.Fatal(err)
	}
